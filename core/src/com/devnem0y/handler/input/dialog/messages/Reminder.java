...
public class Reminder extends Dialog {

    private Texture label;

    public Reminder(int width, int height, String leader, String text, int align, boolean onExit, Stage stage) {
        super(width, height, leader, text, align, onExit, stage);
        label = new Texture("image/reminder.png");
    }

    @Override
    public void show(String text, boolean autoHide, float delaySeconds) {
        super.show(text, autoHide, delaySeconds);
